...
def rundownload(access_token, runid, ids=None, limit=1024, offset=0, sortby='Id', sortdir='Asc'):
    ...
    for response in responses:
        for item in response['Response']['Items']:
            ...
            files_obj = restrequest(sample_json_obj['Response']['HrefFiles'], access_token)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Download all fastq files from BaseSpace for a given run')
    ...
    parser.add_argument('--limit', nargs='?', type=int,
                        help='Number of samples to retrieve', default=1024)
    parser.add_argument('--offset', nargs='?', type=int,
                        help='Offset to begin retrieving samples', default=0)
    parser.add_argument('--sortby', nargs='?', type=str, help='Sort by Id or by DateCreated', default='Id')
    parser.add_argument('--sortdir', nargs='?', type=str, help='Sort (Asc)ending or (Desc)ending', default='Asc')
