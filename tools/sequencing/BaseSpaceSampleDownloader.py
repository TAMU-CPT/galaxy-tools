...
json_obj = restrequest("v1pre3/samples/%s" % SAMPLE_ID)

# Makes a request for the Sample's Files (Location of FASTQ file information)
files_obj = restrequest(json_obj["Response"]["HrefFiles"])
...
for file in files_obj["Response"]["Items"]:
    ...
    if (
        ...
        log.warning(
            "%s exists but may be a partial download / different file" % safe_filename
        )
        sys.exit()
