...
json_obj = restrequest('v1pre3/users/current/runs/')

for run in json_obj['Response']['Items']:
    cols = ['Id', 'DateCreated', 'ExperimentName', 'Status', 'PlatformName']
    data = [run[y] for y in cols]
    data += [run['UserOwnedBy']['Name']]
    print('\t'.join(data))
