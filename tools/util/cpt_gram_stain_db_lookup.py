...
class GramStainDetector(object):
    def __init__(self):
        self.gram_stain_list = {
            "positive": [
    ...
    def expand_search_list(self):
        ...
        self.gram_stain_list["negative"] = map(
            lambda x: x.lower(), self.gram_stain_list["negative"]
        )
        self.gram_stain_list["positive"] = map(
            lambda x: x.lower(), self.gram_stain_list["positive"]
        )

        # Then do list expansions
        negative_expand = []
        for strain in self.gram_stain_list["negative"]:
            parts = strain.split(" ")
            for i in range(len(parts) - 1):
                negative_expand.append(" ".join(parts[i:]))
        for i in negative_expand:
            if i not in self.gram_stain_list["negative"]:
                self.gram_stain_list["negative"].append(i)

        positive_expand = []
        for strain in self.gram_stain_list["positive"]:
            parts = strain.split(" ")
            for i in range(len(parts) - 1):
    ...
    def annotate_name(self, line):
        ...
        if line in self.gram_stain_list["negative"]:
            return "negative"
        elif line in self.gram_stain_list["positive"]:
            return "positive"
        else:
            ...
            if " " in line:
                if line.split(" ")[0] in self.gram_stain_list["negative"]:
                    return "negative"
                elif line.split(" ")[0] in self.gram_stain_list["positive"]:
                    return "positive"
                else:
                    return "not found"
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Single column list of bacterial strains",
    )
    ...
    for line in args.table:
        print "\t".join(line.strip(), gsd.annotate_name(line)) + "\n"
