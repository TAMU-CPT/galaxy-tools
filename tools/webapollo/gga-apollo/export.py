    ...
    for org_ind in org_cn:
        ...
        if args.gff:
            ...
            if "error" in uuid_gff or "uuid" not in uuid_gff:
                raise Exception(
                    "Apollo failed to prepare the GFF3 file for download: %s" % uuid_gff
                )
        ...
        if args.seq:
            ...
            if "error" in uuid_fa or "uuid" not in uuid_fa:
                raise Exception(
                    "Apollo failed to prepare the FASTA file for download: %s" % uuid_fa
                )
        ...
        if args.vcf:
            ...
            if "error" in uuid_vcf or "uuid" not in uuid_vcf:
                raise Exception(
                    "Apollo failed to prepare the VCF file for download: %s" % uuid_vcf
                )
        ...
        if args.fasta_cdna:
            ...
            if "error" in uuid_fa or "uuid" not in uuid_fa:
                raise Exception(
                    "Apollo failed to prepare the cdna FASTA file for download: %s"
                    % uuid_fa
                )
        ...
        if args.fasta_cds:
            ...
            if "error" in uuid_fa or "uuid" not in uuid_fa:
                if "null object" in uuid_fa["error"]:
                    ...
                    raise Exception(
                        "Apollo failed to prepare the cds FASTA file for download: %s"
                        % uuid_fa
                    )
            else:
        ...
        if args.fasta_pep:
            ...
            if "error" in uuid_fa or "uuid" not in uuid_fa:
                raise Exception(
                    "Apollo failed to prepare the file for download: %s" % uuid_fa
                )
