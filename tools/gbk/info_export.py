    ...
    for record in SeqIO.parse(genbank_file, "genbank"):
        ...
        if section == "length":
            ...
            if section == "references":
                output += references_handler(record.annotations[section], record.id)
            elif section in ["comment"]:
                output += comment_handler(record.annotations[section], record.id)
            elif section in ["source", "taxonomy", "date", "organism"]:
                output += tabular_handler(record.annotations[section], record.id)
            else:
                log.error(
                    "Cannot handle section type %s. " "Please submit a feature request",
                    section,
                )
        yield output
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "genbank_files", nargs="+", type=argparse.FileType("r"), help="Genbank file"
    )
    parser.add_argument(
        "--section", type=str, help="Section to export", default="taxonomy"
    )
