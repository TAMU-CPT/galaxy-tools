...
def create_string(length, data, keys):
    data = [("  " * length) + '%s "%s"' % (key, data[key]) for key in keys]
    return ",\n".join(data)
...
if __name__ == "__main__":
    ...
    with open(basename + ".tbl", "w") as handle:
        ...
        for feature in record.features:
            ...
            if feature.strand == -1:
                handle.write(
                    "{0}\t{1}\t{2}\n".format(
                        feature.location.end, feature.location.start + 1, feature.type
                    )
                )
            else:
                handle.write(
                    "{0}\t{1}\t{2}\n".format(
                        feature.location.start + 1, feature.location.end, feature.type
                    )
                )
    ...
    with open(basename + ".sbt", "w") as handle:
        contact_string = create_contact_string(
            4, args.genbank_record_contact, directory
        )
        affil_string = create_affil_string(
            4, args.genbank_record_affiliation, directory
        )
        affil_string2 = create_affil_string(5, args.paper_record_affiliation, directory)
        record_author_str = create_contact_string(
            4, args.genbank_record_author, directory
        )
        ...
        handle.write(
            SBT_TPL % (contact_string, affil_string, record_author_str, affil_string)
        )
        handle.write(
            SBT_TPL2
            % (publication_status, paper_author_str, affil_string2, publication_title)
        )
