...
def genbank_subsection(
    genbank_files, table=None, coordinates=None, include_unlisted=False
):
    if (table is None and coordinates is None) or (
        table is not None and coordinates is not None
    ):
        raise Exception(
            "Must specify a coordinate table (X)OR enter coordinates manually"
        )
            ...
            if table is not None:
                ...
                if rid in cut_sites:
                    for (start, end) in cut_sites[rid]:
                        record.features = [
                            gen_annotation(record, start - 1, end)
                        ] + record.features
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract subsets of genbank files", epilog=""
    )
    parser.add_argument(
        "genbank_files", nargs="+", type=argparse.FileType("r"), help="Genbank files"
    )
    parser.add_argument(
        "--table", type=argparse.FileType("r"), help="Table of coordinates to cut"
    )
