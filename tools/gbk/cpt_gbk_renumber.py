...
def renumber_genes(gbk_file=None, tag_to_update="locus_tag",
    ...
    for i in range(len(records)):
        ...
        for feature_list in sorted(f_care_about, key=lambda x: x[0].location.start):
            for f in feature_list:
                f.qualifiers[tag_to_update] = [format_string % index]
                clean_features.append(f)
                delta['Sheet1']['data'].append([
                    delta_old(f, tag_to_update), format_string % index ])
            index += 1

        # Update all features
        records[i].features = sorted(clean_features, key=lambda x: x.location.start)
    return (records, delta)

def delta_old(feature, tag_to_update):
    # First part of delta entry, old name
    val = '%s %s %s' % (feature.location.start, feature.location.end,
                        feature.location.strand,)
    if tag_to_update in feature.qualifiers:
        val = feature.qualifiers[tag_to_update][0]
    return val
