...
def main(inFile, makeMRNA, outFile):
    ...
    for rec in SeqIO.parse(inFile, "genbank"):
        ...
        for locus in locBucket.keys():
          ...
          if not midFeat and topFeat.type == "gene" and makeMRNA:
              if "locus_tag" in topFeat.qualifiers.keys():
                tempName = topFeat.qualifiers["locus_tag"][0] + ".mRNA"
                tempQuals = {"locus_tag" : topFeat.qualifiers["locus_tag"], "ID" : [tempName], "Notes" : ["mRNA feature automatically generated by Gbk to GFF conversion"]}
              else:
          ...
          if midFeat: # Again, need a new if statement if we want to handle multiple mid-tier features
              ...
              for x in bottomFeats:
                typeDict[x.type] += 1
                outFeats[-1].sub_features.append(makeGffFeat(x, typeDict[x.type] - 1, recID))
                outFeats[-1].sub_features[-1].qualifiers["Parent"] = [outFeats[-1].id]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--outFile', type=argparse.FileType("w"),  help='Path to an output GFF file to be created' )
