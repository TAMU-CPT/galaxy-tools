...
def score_rbs(seq):
    ...
    if "GGAGGA" in (s[5:11], s[6:12], s[7:13], s[8:14], s[9:15], s[10:16]):
        ...
        "GGAAGA" in (s[3:9], s[4:10])
        or "GGATGA" in (s[3:9], s[4:10])
        or "GGACGA" in (s[3:9], s[4:10])
    ):
        score = 18
    elif (
        "GGTGGA" in (s[3:9], s[4:10])
        or "GGGGGA" in (s[3:9], s[4:10])
        or "GGCGGA" in (s[3:9], s[4:10])
    ):
        score = 18
    elif (
        "GGAAGA" in (s[11:17], s[12:18])
        or "GGATGA" in (s[11:17], s[12:18])
        or "GGACGA" in (s[11:17], s[12:18])
    ):
        score = 17
    elif (
        "GGTGGA" in (s[11:17], s[12:18])
        or "GGGGGA" in (s[11:17], s[12:18])
        or "GGCGGA" in (s[11:17], s[12:18])
    ):
        score = 17
        ...
        "AGGA" in (s[11:15], s[12:16])
        or "GAGG" in (s[11:15], s[12:16])
        or "GGAG" in (s[11:15], s[12:16])
    ):
        score = 12
    elif (
        "AGGA" in (s[3:7], s[4:8])
        or "GAGG" in (s[3:7], s[4:8])
        or "GGAG" in (s[3:7], s[4:8])
    ):
        score = 11
    elif (
        "GAGGA" in (s[13:18], s[14:19], s[15:20])
        or "GGAGG" in (s[13:18], s[14:19], s[15:20])
        or "GGAGGA" in (s[13:19], s[14:20], s[15:21])
    ):
        score = 10
        ...
        "GGTGG" in (s[3:8], s[4:9])
        or "GGGGG" in (s[3:8], s[4:9])
        or "GGCGG" in (s[3:8], s[4:9])
    ):
        score = 8
    elif (
        "GGTGG" in (s[11:16], s[12:17])
        or "GGGGG" in (s[11:16], s[12:17])
        or "GGCGG" in (s[11:16], s[12:17])
    ):
        score = 7
    elif (
        "AGG" in (s[11:14], s[12:15])
        or "GAG" in (s[11:14], s[12:15])
        or "GGA" in (s[11:14], s[12:15])
    ):
        score = 6
    elif (
        "GAAGA" in (s[3:8], s[4:9])
        or "GATGA" in (s[3:8], s[4:9])
        or "GACGA" in (s[3:8], s[4:9])
    ):
        score = 5
    elif (
        "GAAGA" in (s[11:16], s[12:17])
        or "GATGA" in (s[11:16], s[12:17])
        or "GACGA" in (s[11:16], s[12:17])
    ):
        score = 4
    elif (
        "AGGA" in (s[13:17], s[14:18], s[15:19])
        or "GAGG" in (s[13:17], s[14:18], s[15:19])
        or "GGAG" in (s[13:17], s[14:18], s[15:19])
    ):
        score = 3
    elif (
        "AGG" in (s[13:16], s[14:17], s[15:18])
        or "GAG" in (s[13:16], s[14:17], s[15:18])
        or "GGA" in (s[13:16], s[14:17], s[15:18])
    ):
        score = 2
    elif (
        "GGAAGA" in (s[13:19], s[14:20], s[15:21])
        or "GGATGA" in (s[13:19], s[14:20], s[15:21])
        or "GGACGA" in (s[13:19], s[14:20], s[15:21])
    ):
        score = 2
    elif (
        "GGTGG" in (s[13:18], s[14:19], s[15:20])
        or "GGGGG" in (s[13:18], s[14:19], s[15:20])
        or "GGCGG" in (s[13:18], s[14:19], s[15:20])
    ):
        score = 2
    elif (
        "AGG" in (s[3:6], s[4:7])
        or "GAG" in (s[3:6], s[4:7])
        or "GGA" in (s[3:6], s[4:7])
    ):
        score = 1
    return score
...
def get_orfs(dna):
        ...
        if codon in start_codons:
            ...
            for start in starts[-frame]:
                ...
                if length >= my_orfs.min_orf_len:
                    ...
                    my_orfs.add_orf(
                        start - 2, stop, length, -frame, seq, rbs, rbs_score
                    )
                    training_rbs[rbs_score] += 1
    ...
    for frame in [1, 2, 3]:
        ...
        if rev_comp(dna[start - 3 : start]) not in start_codons:
            starts[-frame].append(
                my_orfs.contig_length - ((my_orfs.contig_length - (frame - 1)) % 3)
            )
        for start in starts[-frame]:
            stop = stops[-frame]
            length = start - stop + 1
    ...
    for orf in my_orfs.iter_orfs():
        ...
        if orf.frame > 0:
            for base in range(start, stop, 3):
                ind_max = max_idx(
                    gc_pos_freq[base][0], gc_pos_freq[base][1], gc_pos_freq[base][2]
                )
                ind_min = min_idx(
                    gc_pos_freq[base][0], gc_pos_freq[base][1], gc_pos_freq[base][2]
                )
                orf.hold = orf.hold * (
                    ((1 - orf.pstop) ** pos_max[ind_max]) ** pos_min[ind_min]
                )
        else:
            for base in range(start, stop, -3):
                ind_max = max_idx(
                    gc_pos_freq[base][2], gc_pos_freq[base][1], gc_pos_freq[base][0]
                )
                ind_min = min_idx(
                    gc_pos_freq[base][2], gc_pos_freq[base][1], gc_pos_freq[base][0]
                )
...
def get_graph(my_orfs):
                    ...
                    for left_node in G.iternodes():
                        ...
                        if last + 1 >= l > last - 300 and base - 1 <= r < base + 300:
                            if left_node.frame * right_node.frame > 0:
                                if (
                                    left_node.type == "stop"
                                    and right_node.type == "start"
                                    and left_node.frame > 0
                                ):
                                    score = score_gap(r - l - 3, "same", pgap)
                                    G.add_edge(Edge(left_node, right_node, score))
                                elif (
                                    left_node.type == "start"
                                    and right_node.type == "stop"
                                    and left_node.frame < 0
                                ):
                                    score = score_gap(r - l - 3, "same", pgap)
                                    G.add_edge(Edge(left_node, right_node, score))
        ...
        for left_node in G.iternodes():
            ...
            if 0 < r - l < 300:
                ...
                if left_node.gene == "tRNA" or right_node.gene == "tRNA":
                    if (
                        left_node.frame * right_node.frame > 0
                        and left_node.type != right_node.type
                    ):
                        if (left_node.frame > 0 and left_node.type == "stop") or (
                            left_node.frame < 0 and left_node.type == "start"
                        ):
    ...
    for node in G.iternodes():
        if node.position <= 2000:
            if (node.type == "start" and node.frame > 0) or (
                node.type == "stop" and node.frame < 0
            ):
