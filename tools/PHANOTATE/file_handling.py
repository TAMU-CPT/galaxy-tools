...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description="PHANOTATE: A phage genome annotator",
    ...
    parser.add_argument(
        ...
        help="where to write the output [stdout]",
    )
    parser.add_argument(
        "-f",
        "--outfmt",
        ...
        default="tabular",
        dest="outfmt",
        help="format of the output [tabular]",
        choices=["tabular", "genbank", "fasta"],
    )
    ...
    if not my_path:
        ...
        for source, target in pairwise(my_path):
            ...
            if left.position == 0 and right.position == last_node.position:
                ...
                right.position = ">" + str(
                    left.position + 3 * int((right.position - left.position) / 3) - 1
                )
        ...
        for source, target in pairwise(my_path):
            ...
            if left.gene == "CDS":
                ...
                if left.position == 0 and right.position == last_node.position:
                    ...
                    right.position = ">" + str(
                        left.position
                        + 3 * int((right.position - left.position) / 3)
                        - 1
                    )
                    left.position = "<" + str(left.position)
                elif left.position == 0:
