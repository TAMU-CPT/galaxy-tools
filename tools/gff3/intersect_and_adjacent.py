...
def treeFeatures(features, window):
    for feat in features:
        ...
        yield Interval(int(feat.location.start) - int(window), int(feat.location.end) + int(window), feat.id)
    ...
    if len(rec_a) > 0 and len(rec_b) > 0:
        ...
        if stranding == True:
          for feat in rec_a.features:
            if feat.strand > 0:
              a_pos.append(Interval(int(feat.location.start) - int(window), int(feat.location.end) + int(window), feat.id))
            else:
              a_neg.append(Interval(int(feat.location.start) - int(window), int(feat.location.end) + int(window), feat.id))

          for feat in rec_b.features:
            if feat.strand > 0:
              b_pos.append(Interval(int(feat.location.start) - int(window), int(feat.location.end) + int(window), feat.id))
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-stranding', action='store_true', help='Only allow adjacency for same-strand features')
