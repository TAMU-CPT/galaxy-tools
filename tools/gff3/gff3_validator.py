...
def table_annotations(gff3In, out_errorlog):
  ...
  for record in list(GFF.parse(gff3In)):
    ...
    for featLvl1 in record.features:
      ...
      if dupCheck != -1:
        errorMessage = errorMessage + ("Error: Duplicate %ss in GFF3 file\n  %s ID %s occupies the same space as %s ID: %s\n  Note: if one of these IDs doesn't appear to exist, ensure there are not two of the other\n\n" % (featLvl1.type, dupCheck.type, dupCheck.id, featLvl1.type, featLvl1.id))
        numError += 1
      else:
        containsHash[(featLvl1.type + str(featLvl1.location.start) + str(featLvl1.location.end))] = featLvl1
      ...
      if (featLvl1.type == "gene"):
        ...
        for featLvl2 in featLvl1.sub_features:
          ...
          if dupCheck != -1:
            errorMessage = errorMessage + ("Error: Duplicate %ss in GFF3 file\n  %s ID %s occupies the same space as %s ID: %s\n  Note: if one of these IDs doesn't appear to exist, ensure there are not two of the other\n\n" % (featLvl2.type, dupCheck.type, dupCheck.id, featLvl2.type, featLvl2.id))
            numError += 1
          else:
            containsHash[(featLvl2.type + str(featLvl2.location.start) + str(featLvl2.location.end))] = featLvl2

          if (featLvl2.type == "mRNA"):
            foundMRNA += 1

            if(featLvl2.location.start != featLvl1.location.start):
              errorMessage = errorMessage + "Error: mRNA start boundary does not match parent gene's boundary\n  Gene %s Start Boundary: %d\n  mRNA %s Start Boundary: %d\n\n" % (featLvl1.id, featLvl1.location.start, featLvl2.id, featLvl2.location.start)
              numError += 1

            if(featLvl2.location.end != featLvl1.location.end):
              errorMessage = errorMessage + "Error: mRNA end boundary does not match parent gene's boundary\n  Gene %s End Boundary: %d\n  mRNA %s End Boundary: %d\n\n" % (featLvl1.id, featLvl1.location.end, featLvl2.id, featLvl2.location.end)
              numError += 1
            ...
            for featLvl3 in featLvl2.sub_features:
              ...
              if dupCheck != -1:
                errorMessage = errorMessage + ("Error: Duplicate %ss in GFF3 file\n  %s ID %s occupies the same space as %s ID: %s\n  Note: if one of these IDs doesn't appear to exist, ensure there are not two of the other\n\n" % (featLvl3.type, dupCheck.type, dupCheck.id, featLvl3.type, featLvl3.id))
                numError += 1
              else:
                containsHash[(featLvl3.type + str(featLvl3.location.start) + str(featLvl3.location.end))] = featLvl3
              ...
              if(featLvl3.type == "exon"):
                ...
                if(featLvl3.location.start < featLvl2.location.start or featLvl3.location.start > featLvl2.location.end):
                  errorMessage += ("Error: CDS start falls outside the boundary of parent mRNA\n  %s mRNA Start: %d  -- End: %d\n  %s CDS Start: %d\n\n" % (featLvl2.id, featLvl2.location.start, featLvl2.location.end, featLvl3.id, featLvl3.location.start))
                  numError += 1
                elif(featLvl3.location.end < featLvl2.location.start or featLvl3.location.end > featLvl2.location.end):
                  errorMessage += ("Error: CDS end falls outside the boundary of parent mRNA\n  %s mRNA Start: %d  -- End: %d\n  %s CDS End: %d\n\n" % (featLvl2.id, featLvl2.location.start, featLvl2.location.end, featLvl3.id, featLvl3.location.end))
                  numError += 1
...
#                  errorMessage += ("Error: Shine Dalgarno does not match expected start/end pair based on parent mRNA boundary\n  %s mRNA Start: %d  -- End: %d\n  %s Shine Dalgarno Start: %d  -- End: %d\n\n" % (featLvl2.id, featLvl2.location.start, featLvl2.location.end, featLvl3.id, featLvl3.location.start, featLvl3.location.end))
            ...
            if cdsList: # Interpreter requires this, can't just check against CDS sum
              if CDS == 1:
                cdsCheck = cdsList.pop()
                if(cdsCheck.location.start != featLvl2.location.start and cdsCheck.location.end != featLvl2.location.end):
                  errorMessage += ("Error: CDS does not match expected start/end pair based on parent mRNA boundary\n  %s mRNA Start: %d  -- End: %d\n  %s CDS Start: %d  -- End: %d\n\n" % (featLvl2.id, featLvl2.location.start, featLvl2.location.end, cdsList[0].id, cdsList[0].location.start, cdsList[0].location.end))
                  numError += 1
              elif (CDS > 1):
                match = False
                for i in CDS:
                  cdsCheck = cdsList.pop()
                  if cdsCheck.location.start != featLvl2.location.start or cdsCheck.location.end != featLvl2.location.end:
                    match = True
                if match == False:
...
#              errorMessage = ("Error: %s mRNA has %d Shine Dalgarno sequences, expected no more than 1\n\n" % (featLvl2.id, shines))  + errorMessage
            ...
            if(featLvl2.location.start < featLvl1.location.start or featLvl2.location.start > featLvl1.location.end):
              errorMessage += ("Error: %s start falls outside the boundary of parent gene\n  %s gene Start: %d  -- End: %d\n  %s %s Start: %d\n\n" % (featLvl2.type, featLvl1.id, featLvl1.location.start, featLvl1.location.end, featLvl2.id, featLvl2.type, featLvl2.location.start))
              numError += 1
            elif(featLvl2.location.end < featLvl1.location.start or featLvl2.location.end > featLvl1.location.end):
              errorMessage += ("Error: %s end falls outside the boundary of parent gene\n  %s gene Start: %d  -- End: %d\n  %s %s End: %d\n\n" % (featLvl2.type, featLvl1.id, featLvl1.location.start, featLvl1.location.end, featLvl2.id, featLvl2.type, featLvl2.location.end))
              numError += 1
            elif(featLvl2.location.start != featLvl1.location.start and featLvl2.location.end != featLvl1.location.end):
              errorMessage += ("Error: %s does not match expected start/end pair based on parent gene boundary\n  %s gene Start: %d  -- End: %d\n  %s %s Start: %d  -- End: %d\n" % (featLvl2.type, featLvl1.id, featLvl1.location.start, featLvl1.location.end, featLvl2.id, featLvl2.type, featLvl2.location.start, featLvl2.location.end))
              numError += 1
            errorMessage = ("Error: %s sub-feature paired to gene and not mRNA feature\n\n" % featLvl2.type) + errorMessage
            numError +=1
        if foundMRNA > 1:
          errorMessage = ("Error: Gene %s has more than one mRNA sub-feature\n\n" % (featLvl1.id)) + errorMessage
          numError += 1
        elif foundMRNA == 0:
          errorMessage = ("Warning: Gene %s has no mRNA Feature\n\n" % (featLvl1.id)) + errorMessage
          numWarning += 1
  ...
  out_errorlog.write("Finished with %d Errors and %d Warnings" % (numError, numWarning))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Validate that contents of GFF3 are correct')
    parser.add_argument('gff3In', type=argparse.FileType("r"), help='GFF3 source file')
    parser.add_argument('--out_errorlog', type=argparse.FileType("w"), help='Output Error Log', default='test-data/errorlog.txt')
