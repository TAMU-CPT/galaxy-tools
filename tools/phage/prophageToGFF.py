...
def convertTable(prophageIn, fastaIn):
    ...
    for row in prophageIn:
        ...
        if len(line) != 7 or row.startswith("Accession Number"):
            continue
        
        geneName = line[0]
        geneStart = min(int(line[2]), int(line[3]))
        geneEnd = max(int(line[2]), int(line[3]))
        geneQuals ="ID=%s;Name=%s;Score=%s;Notes=" % (geneName, geneName, line[1])
        for x in line[6]:
          try:
        ...
        geneQuals = geneQuals + validArray[1] + " contains %s HSPs;" % line[5]
