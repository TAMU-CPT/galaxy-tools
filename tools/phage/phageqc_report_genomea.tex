...
You have {{ weird_starts_bad }} unusual start codons. The start codon distribution is as follows:
...
You may wish to investigate the following starts individually and verify their correctness.
...
{% for row in excessive_overlap %}
    {{row[0].id}} & \texttt{{'{'}}{{row[0].location}}{{'}'}} & {{row[1].id}} & \texttt{{'{'}}{{row[1].location}}{{'}'}} & {{row[2]}}..{{row[3]}} & {{row[3] - row[2]}} \\
{% endfor %}
\end{longtable}
