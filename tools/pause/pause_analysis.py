...
def identify_regions(peak_f, peak_r, data_f, data_r, cov_f, cov_r, gl=0):
    ...
    report = [
        '# PAUSE Report',
        '',
        'This report was automatically generated by the PAUSE software. ' +
        'It merely records some information about the analysis process',
        '',
        ...
        for pr in peak_r[:, 0]:
            ...
            if (pf < pr and pr - pf < (.5 * gl)) or \
                ...
                if between > (outside * 1.75):
                    findings.append(' - Found possible region on [%s..%s], where the average coverage (%s)' +
                                    ' is larger than the rest of the genome (%s)' % (pf, pr, between, outside))
                    conclusions.append(' - Region [%s..%s] is likely candidate for a repeat_region' % (pf, pr))
                    concl_base.append([pf, pr])
                else:
                    findings.append(' - Discounted region [%s..%s], as average coverage (%s) was not ' +
                                    'significantly larger than the rest of the genome (%s)'
                                    % (pf, pr, between, outside))
            else:
    ...
    if len(conclusions) == 3:
        ...
        recommendations.append('Multiple possible regions were found, you should manually inspect these regions ' +
                               'to narrow them down to a single correct location for a repeat')
