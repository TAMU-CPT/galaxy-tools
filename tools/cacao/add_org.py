...
def get(token, url):
    q = requests.get(url, headers={"Authorization": token}).json()
    return q
...
def main():
    ...
    organism = post(
        token,
        args.url + "organisms/",
        dict(common_name=organism_name, taxon=args.taxon, ebi_id=args.ebi_id),
    )
    if (
        organism.get("common_name", [None])[0]
        == "organism with this common name already exists."
    ):
        organism = get(token, args.url + "organisms/?common_name=" + organism_name)[
            "results"
        ][0]
    ...
    for record in SeqIO.parse(args.fasta, "fasta"):
        refseq = post(
            ...
            args.url + "refseq/",
            dict(  # noqa
                name=record.id, length=len(record.seq), organism=organism["id"]
            ),
        )
        if (
            refseq.get("non_field_errors", [None])[0]
            == "The fields name, organism must make a unique set."
        ):
            refseq = get(
                token, args.url + "refseq/?name=%s&%s" % (record.id, organism["id"])
            )["results"][0]
