...
def reduce_subfeatures(subfeatures):
    ...
    for feature in subfeatures:
        ...
        if (
            feature.location.start == prev_feature.location.end
            and prev_feature.qualifiers["score"] == feature.qualifiers["score"]
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Fasta file (in same order) passed to parent for reconstructing proper IDs",
    )

    parser.add_argument(
        "--window_size", type=int, help="Window size for analysis", default=1000
    )
