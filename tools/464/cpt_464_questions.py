...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="464 Questionnaire",
        epilog="""Fill out questions, run as many times as you need!
...
These questions are intended to reinforce key concepts that should be part of your GenomeA papers.""",
    ...
    parser.add_argument(
        "--cdss_assigned", help="# of CDSs with assigned functions", type=int
    )
    parser.add_argument("--cdss_hypo_novel", help="# of Hypothetical Novels", type=int)
    parser.add_argument(
        "--cdss_hypo_consv", help="# of Hypothetical Conserveds", type=int
    )
    ...
    parser.add_argument(
        "--missing_core",
        help="Are there any core genes missing or out of place? "
        + "Identify and explain.",
    )
    parser.add_argument(
        "--no_phage_homos",
        help="Do you have any gene with assigned function that"
    ...
    parser.add_argument(
        "--unusual",
        nargs="+",
        help="What unusual or unexpected genes or genomic "
    ...
    parser.add_argument(
        "--no_sd",
        nargs="+",
        help="Do you have any genes without a legitimate (se"
    ...
    parser.add_argument(
        "--gaps",
        nargs="+",
        help="Justify any intergenic gap larger than 150 bp. "
        + "(explain why you did not annotate genes?)",
    )
    parser.add_argument(
        "--closed",
        help="Is your genome closed? How did you determine this? (link"
        + " to gel images & analysis text , including primer sequence).",
    )
    parser.add_argument(
        "--reopened",
        help="Has your phage genome been re-opened? If so, where and"
        + " why? If not, why not?",
    )
    parser.add_argument(
        "--sim",
        help="Is your phage similar to another phage? Which one? How simi"
        + "lar? How was the similarity determined?",
    )
    parser.add_argument(
        "--gc_skew",
        help="Does your phage exhibit GC skew? Can you approximate an"
    ...
    fixed_vars["unusual"] = (
        "<ul><li>" + "</li><li>".join(fixed_vars["unusual"]) + "</li></ul>"
    )
    fixed_vars["no_sd"] = (
        "<ul><li>" + "</li><li>".join(fixed_vars["no_sd"]) + "</li></ul>"
    )
    fixed_vars["gaps"] = (
        "<ul><li>" + "</li><li>".join(fixed_vars["gaps"]) + "</li></ul>"
    )
