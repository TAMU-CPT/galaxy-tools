...
if __name__ == "__main__":
    ...
    subprocess.check_call(
        [
            "tar",
            "xfz",
            os.path.join(SCRIPT_DIR, "test-data", "assessment-2.tgz"),
            "--strip-components",
            "1",
            "-C",
            outdir,
    ...
    data = wa.organisms.addOrganism(org_cn, outdir, genus="", species="", public=False)
    ...
    sys.stdout.write(
        HTML_TPL.format(
            base_url="https://cpt.tamu.edu/apollo", chrom="", orgId=data[0]["id"]
        )
    )
