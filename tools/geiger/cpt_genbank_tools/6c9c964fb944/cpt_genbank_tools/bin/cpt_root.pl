...
my $options = $libCPT->getOptions(
	'options' => [
		[
			'protein' => 'Input file containing a single protein sequence',
			{
				validate    => 'File/Input',
			}
		],
...
sub parse{
	...
	my $c = 0;
	my $seqio_object = $bio->getSeqIO($protein_results);
		...
		for my $key ( $anno_collection->get_all_annotation_keys ) {
			...
			for my $value ( @annotations ) {
				if($value->tagname eq 'date_changed'){
                    ...
                    my $date = $strp->parse_datetime($value->display_text);
                    if(defined($date) && defined($value->display_text)){
                            $ids{$acc}{'submission_date'} = $date;
                    }else{
                            $ids{$acc}{'submission_date'} = $strp->parse_datetime('01-Jan-0000');
                    }
				}
...
sub report{
	...
	foreach($years[0]..$years[-1]){
		if($date_data{$_}){
			$result .=  sprintf("    %s [%s] %s\n", $_, int($date_data{$_}),  '*' x int(log(int($date_data{$_}))));
		}
	}
	$result .=  "\n\n";

	$result .=  "# Full Report\n";
	@acc_sorted = sort { DateTime->compare($ids{$a}{'submission_date'},$ids{$b}{'submission_date'}) } map { if(defined($ids{$_}{'submission_date'})){ $ids{$_} }else{ () } }keys %ids;
	foreach my $acc (@acc_sorted){
		$result .=  "## $acc (" . defined $ids{$acc}{submission_date} ? $ids{$acc}{submission_date}->strftime("%Y-%m-%d") : "UNKNOWN" . ")\n";
