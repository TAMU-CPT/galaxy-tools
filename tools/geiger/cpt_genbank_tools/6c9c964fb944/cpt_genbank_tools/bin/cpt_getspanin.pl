...
my $options = $libCPT->getOptions(
		'options' => [
			...
			[ 'file', 'Input genome sequence as fasta file', { required => 1, validate => 'File/Input' } ],
			...
			[ 'isp_nterm'  , 'distance to first AA of tmd of ispanin (in codons)'     , { required => 1 , validate => 'Int' , default => 10 } ] ,
			[ 'osp_min'    , 'smallest ORF length for o-spanin (in codons)'           , { required => 1 , validate => 'Int' , default => 30 } ] ,
			[ 'osp_signal' , 'distance from start to first AA of lipobox (in codons)' , { required => 1 , validate => 'Int' , default => 30 } ] ,
			[],
			['Extra'],
	...
	foreach my $pmi ( @isps ) {
		...
		if ( scalar @hits > 0 ) {
			...
			foreach my $osp_hit_obj (@hits) {
				...
				push @simulated_fasta_thingy, " " x $diff . $oseq;
			}
			$response .= p("Information on ISPs and possible OSPs");
			foreach my $osp_hit_obj (@hits) {
				my ( $osp_hit, $type ) = @{$osp_hit_obj};
				my ( $olen, $ostart, $oend, $ostrand, $oseq ) = convert_to_old_format($osp_hit);
			}
...
sub filter_osps_for_lipoboxes {
	...
	my @lipoboxes = (
		qr/[LVWYF][NCQMST][GAS]C/    # (L-V-or other large hydrophobic)-X (not charged) – (G or A or S) – C
	...
	for ( my $i = 0 ; $i < scalar @sequences; $i++ ) {
		...
		if ( defined $lookup_tmds ) {
			...
			if ( !defined($first_start) ) {
				$first_start = $options->{isp_nterm};
			}
			if ( $first_start < $options->{isp_nterm} ) {
				push(@good_sequences, $individual_seq);
			}
		}
		else {
