#!/usr/bin/env perl
# PODNAME: pause_read-sam-zoom.pl
use strict;
use Bio::DB::Sam;
...
PAUSE:Plot:Zoom is a tool to generate enlarged subsets of PAUSE:Plot images. No automated analysis is done with this tool.
...
=item C<from> base start of the subset you wish to plot

=item C<to> base end of the subset you wish to plot
...
my $options = $ggo->getOptions(
	'options' => [
		...
		[
			'sense_bam',
			'Sense BAM File/Input',
			{ required => 1, validate => 'File/Input' }
		],
		[
			'antisense_bam',
			'Antisense BAM File/Input',
			{ required => 1, validate => 'File/Input' }
		],
		[ 'from', 'From', { required => 1, validate => 'Int', default => 1000} ],
		[ 'to',   'To',   { required => 1, validate => 'Int', default => 2000 } ],
		[],
		["Plot Options"],
		['kb_per_row', 'Base number of kb plotted per row', { required => 1, validate => 'Int', default => 20 }],
		['line_height', 'Vertical size of each section of the genome', { required => 1, validate => 'Int', default => 500 }],
	...
	'defaults' => [
		'appid'   => 'read_plotter_zoom',
		'appname' => 'Aligned BAM PAUSE Plotter (Zoom)',
		'appdesc' =>
