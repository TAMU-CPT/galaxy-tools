...
my $options = $libCPT->getOptions(
	'options' => [
		...
		[ 'start', 'Start of subset plot', { required => 1, validate => 'Int'}],
		[ 'end', 'End of subset plot', { required => 1, validate => 'Int'}],
		[],
		[
...
my %color_scheme = %{ $libCPT->JSONYAMLopts( 'file' => $color_scheme ) };
...
my $svg_control = CPT::Plot::Base->new(
	...
	'features'     => \@features,
	'genome_length'=> $options->{end}-$options->{start},
	#'file'         => $options->{file},
